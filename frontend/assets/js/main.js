document.addEventListener("DOMContentLoaded", () => {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            addMessage(data.reply || "Kechirasiz, javob topolmadim.", "ai");
        } catch (err) {
            removeTypingIndicator();
            addMessage("Ulanishda xato yuz berdi 📞 +998 97 925 20 03", "ai");
        } finally {
    ...
    setTimeout(() => { if (!isOpen) toggleChat(); }, 1500);
});
...
(() => {
    function showToast(msg, type) {
        ...
        t.id = "sa-toast";
        const clr = type === "error" ? "background:#fe6b00;color:#fff" : "background:#b0ff96;color:#0e0e0e";
        t.setAttribute("style", `position:fixed;top:96px;right:32px;z-index:9999;${clr};padding:14px 22px;border-radius:14px;font-weight:700;font-size:14px;display:flex;align-items:center;gap:10px;box-shadow:0 8px 32px rgba(0,0,0,.2);transition:opacity .4s,transform .4s;font-family:'Space Grotesk',sans-serif`);
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        phoneInp.addEventListener("keydown", (e) => {
            if ([46, 8, 9, 27, 13].indexOf(e.keyCode) !== -1 || (e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode >= 96 && e.keyCode <= 105)) return;
            e.preventDefault();
        });
        ...
        async function submitContact() {
            ...
            if (!name) { showToast("Ismingizni kiriting!", "error"); if(nameInp) nameInp.focus(); return; }
            if (!course) { showToast("Kursni tanlang!", "error"); if(courseInp) courseInp.focus(); return; }
            if (!digits || digits.length < 9) { showToast("Telefon raqamni to'liq kiriting!", "error"); phoneInp.focus(); return; }
            ...
            try {
                ...
                if (r.ok && d.success) {
                    showToast("✅ Arizangiz qabul qilindi!");
                    phoneInp.value = "";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const toggleMenu = () => { if (mobileMenu) mobileMenu.classList.toggle("translate-x-full"); };
